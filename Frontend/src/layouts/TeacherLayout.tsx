...
import {
  BookOpen, LayoutDashboard, PlusCircle, Database,
  BarChart2, LogOut, Menu, X, User
} from 'lucide-react'

const navItems = [
  { to: '/teacher/dashboard',       icon: LayoutDashboard, label: 'Dashboard'       },
  { to: '/teacher/question-bank',   icon: Database,        label: 'Question Bank'   },
  { to: '/teacher/create-exam',     icon: PlusCircle,      label: 'Create Exam'     },
  { to: '/teacher/analytics',       icon: BarChart2,       label: 'Exam Analytics'  },
        ...
        <header className="flex items-center justify-between px-6 py-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-4 ml-auto relative">
            <div className="text-right hidden sm:block">
              <div className="font-bold text-sm text-slate-800">{user?.name || user?.fullName || 'Teacher'}</div>
              <div className="text-xs text-slate-500 capitalize">{user?.role || 'Teacher'}</div>
            </div>
